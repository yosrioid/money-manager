...
import { index, yearly } from '@/routes/budgets';
...
usePeriodNavigation({
    ...
    previousHref: () => yearly({ query: { year: props.previousYear } }).url,
    nextHref: () => yearly({ query: { year: props.nextYear } }).url,
});
</script>

<template>
    <Head title="Yearly Budgets" />

    <div class="flex flex-col gap-8 p-4">
        <Heading
            title="Budgets"
            description="Compare actual spending against your annual budget"
        />

        <BudgetViewNav current="yearly" />

        <div class="flex items-center justify-between gap-4">
            <Button variant="outline" size="sm" as-child>
                <Link :href="yearly({ query: { year: previousYear } })"
                    ><ChevronLeft /> Previous</Link
                >
            </Button>
            <Heading variant="small" :title="year" />
            <Button variant="outline" size="sm" as-child>
                <Link :href="yearly({ query: { year: nextYear } })"
                    >Next <ChevronRight
                /></Link>
            </Button>
        </div>

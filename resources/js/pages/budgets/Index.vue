...
defineOptions({
    layout: { breadcrumbs: [{ title: 'Budgets', href: index() }] },
});
...
const isOverPace = (row: BudgetRow): boolean =>
...
<template>
    ...
    <div class="flex flex-col gap-8 p-4">
        <Heading
            title="Budgets"
            description="Compare actual spending against your monthly budget"
        />

        <BudgetViewNav current="monthly" />

        <div class="flex items-center justify-between gap-4">
            <Button variant="outline" size="sm" as-child>
                <Link :href="index({ query: { month: previousMonth } })"
                    ><ChevronLeft /> Previous</Link
                >
            </Button>
            <Heading variant="small" :title="monthLabel" />
            <Button variant="outline" size="sm" as-child>
                <Link :href="index({ query: { month: nextMonth } })"
                    >Next <ChevronRight
                /></Link>
            </Button>
        </div>

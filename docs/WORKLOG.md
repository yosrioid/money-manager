...
- **Branch:** `feat/phase-5`.
- **Feature IDs:** `P5-06`, `P5-07`.
- **Status:** Completed.
- **Completed:** Added migration `2026_06_14_083514_create_installment_plans_table` creating `installment_plans` (1:1 unique FK to `transactions`, FK to `accounts`, `total_amount`, `installment_count`, `first_due_date`, indexed on `[workspace_id, account_id]`), `App\Models\InstallmentPlan` (`workspace()`/`transaction()`/`account()`, `first_due_date` cast `immutable_date`), `Transaction::installmentPlan()` (`HasOne`), `Account::installmentPlans()` (`HasMany`), and `InstallmentPlanFactory`. New pure domain services: `App\Domain\Accounts\GenerateInstallmentSchedule::generate(totalAmount, installmentCount, firstDueDate)` builds a monthly schedule (via `addMonthsNoOverflow`) distributing the `total_amount % installment_count` remainder across the earliest installments; `App\Domain\Accounts\SummarizeInstallmentPlan::summarize(plan, referenceDate)` combines that schedule with a reference date into `paid_count`/`paid_amount`, the next `due` installment (`null` if fully paid), and `remaining_count`/`remaining_amount`. `StoreTransactionRequest` adds optional `installment_count` (2-60, `prohibited_unless:type,expense`) and `first_due_date` (`required_with:installment_count`, same `prohibited_unless`), plus an `after()` check `validateInstallmentPlan()` rejecting installment plans for split transactions or non-`credit_card` accounts. `TransactionController::store()` now captures `recordSplit()`'s returned `Transaction` and creates the `InstallmentPlan` row when `installment_count` is present; `formProps()`'s `accounts` selection adds `type` so the UI can detect credit-card accounts. `TransactionForm.vue` shows a "Pay in installments" checkbox (installment count / first due date inputs) only for non-split expense transactions on a `credit_card` account. `AccountGroupController::index()` eager-loads `installmentPlans` and exposes per-plan `summarizeInstallmentPlan` summaries for `credit_card` accounts; `accounts/Index.vue` lists each plan's paid/due/remaining progress.
- **Verification:** `php artisan test --compact` passed: 349 tests, 2629 assertions (new: `tests/Unit/GenerateInstallmentScheduleTest.php`, `tests/Feature/SummarizeInstallmentPlanTest.php`, and 5 new tests in `tests/Feature/TransactionRecordingTest.php`). `vendor/bin/phpstan analyse --no-progress --memory-limit=1G` passed (0 errors). `npm run types:check` passed. `vendor/bin/pint --dirty --format agent` passed. ESLint on changed Vue files passed with no output.
- **Decisions:** `SummarizeInstallmentPlanTest` was placed under `tests/Feature/` (not `Unit/`) because constructing an `InstallmentPlan` with an `immutable_date` cast requires Eloquent's connection resolver, which is only bound for the `Feature` test suite (`RefreshDatabase`). `SummarizeInstallmentPlan::summarize()` reads `$plan->getAttributes()['first_due_date']` and parses it with `CarbonImmutable::parse()` rather than `$plan->first_due_date` directly or `getRawOriginal()`, because (a) Larastan infers all `casts()`-based date attributes as `string` regardless of cast type (a pre-existing limitation also affecting `Transaction::occurred_at`), and (b) `getRawOriginal()` returns `null` for not-yet-persisted models (its `original` array is synced before `fill()` in `Model::__construct`), which broke the unsaved-model test fixtures.
- **Blockers:** None.
- **Uncommitted:** All of `P5-06`/`P5-07` is uncommitted on `feat/phase-5`, pending commit.
- **Next:** Commit `P5-06`/`P5-07`, then continue with `P5-08` (loan account: principal liability and repayment transfers are tracked).
